...
export function DisplayNameForm({ displayName, userId }: Props) {
  const queryClient = useQueryClient();

  const updateDisplayName = useMutation({
    mutationFn: async (newDisplayName: string) => {
      const response = await axios.put(`/api/user/settings?userId=${userId}`, {
        name: newDisplayName,
      });
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    updateDisplayName.mutate(newDisplayName);
  };
  ...
  return (
    <Card>
      <form onSubmit={handleSubmit}>
        <CardHeader>
          <CardTitle className="text-xl font-bold">Nom d'affichage</CardTitle>
          <CardDescription>
