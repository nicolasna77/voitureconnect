import { AlertTriangle, TrendingUp, Gauge, Calendar, Fuel, Settings } from "lucide-react";
...
const issues = [
  {
    ...
    description: "Encrassement fréquent, peut causer perte de puissance et voyant moteur.",
    ...
    <section id="demo" className="relative py-24 lg:py-36 overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-16 grid gap-6 lg:grid-cols-2">
          {/* Vehicle Card */}
          <Reveal animation="animate-slide-right" delay={200}>
            <div className="rounded-2xl border border-border bg-card p-6 transition-all duration-300 hover:shadow-lg lg:p-8">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Analyse complète</p>
                  <h3 className="mt-2 text-2xl font-bold text-card-foreground">Peugeot 308 II</h3>
                ...
                <div className="flex flex-col items-center rounded-2xl border border-primary/20 bg-primary/5 px-5 py-3">
                  <span className="text-3xl font-bold text-primary tabular-nums">7.5</span>
                  <span className="text-xs font-medium text-primary/60">/10</span>
                </div>
              </div>

              <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3">
                {specs.map((spec) => (
                  <div key={spec.label} className="rounded-xl border border-border bg-secondary/30 p-3">
                    <spec.icon className="mb-2 h-4 w-4 text-chart-3" aria-hidden="true" />
                    <p className="text-xs text-muted-foreground">{spec.label}</p>
                    <p className="font-semibold text-card-foreground">{spec.value}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-8 rounded-xl border border-border bg-secondary/30 p-5">
                ...
                <div className="mt-2 flex items-baseline gap-3">
                  <span className="text-3xl font-bold text-chart-3 tabular-nums">6 500 €</span>
                  <span className="text-lg text-muted-foreground">—</span>
                  <span className="text-3xl font-bold text-chart-3 tabular-nums">8 200 €</span>
                </div>
          ...
          {/* Known Issues */}
          <Reveal animation="animate-slide-left" delay={350}>
            <div className="rounded-2xl border border-border bg-card p-6 transition-all duration-300 hover:shadow-lg lg:p-8">
              ...
              <div className="mt-6 space-y-3">
                {issues.map((issue) => (
                  <div key={issue.title} className="rounded-xl border border-border bg-secondary/20 p-4">
                    <div className="flex items-center justify-between">
                      <h4 className="font-semibold text-card-foreground">{issue.title}</h4>
                      <span className={`rounded-full border px-2.5 py-1 text-xs font-medium ${issue.severityColor}`}>
                        {issue.severity}
                      </span>
                    </div>
